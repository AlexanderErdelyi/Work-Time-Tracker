...
export function Reports() {
  ...
  const stats = useMemo(() => {
    ...
    const totalBilledHours = parseFloat(entries.reduce((sum, entry) => sum + (entry.billedHours || 0), 0).toFixed(2))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Billed Hours</CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.totalBilledHours}</div>
